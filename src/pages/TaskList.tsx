...
export function TaskList({ onEditTask, onNewTask }: TaskListProps) {
  ...
  const filtered = useMemo(() => {
    ...
    switch (tab) {
      case "today":
        result = result.filter((t) => t.due_date === today && t.status === "pendente");
        break;
      case "upcoming":
        result = result.filter((t) => isUpcoming(t.due_date) && t.status === "pendente");
        break;
      case "completed":
  ...
  const activeFilters =
    (filterCategory ? 1 : 0) + (filterPriority ? 1 : 0) + (filterStatus ? 1 : 0);
    ...
    <div className="p-4 lg:p-6 max-w-5xl mx-auto pb-24 lg:pb-6">
      ...
      <div className="flex gap-2 mb-4">
        ...
        >
          ...
          {activeFilters > 0 && (
            <span className="badge bg-green-600 text-white text-[10px]">{activeFilters}</span>
          )}
        </button>
        <Select
          value={sortBy}
