    ...
    <div className="p-4 lg:p-6 max-w-3xl mx-auto pb-24 lg:pb-6 space-y-6">
      ...
      <div className="card p-5">
        ...
        <div className="flex items-center justify-between">
          ...
          >
            <span
              className={`absolute top-0.5 left-0.5 w-5 h-5 rounded-full bg-white shadow-sm transition-transform ${theme === "dark" ? "translate-x-6" : ""}`}
            />
          </button>
        </div>
      </div>
          ...
          {[
            {
              ...
              desc: "Avisar quando uma tarefa estiver próxima do vencimento",
              enabled: true,
            },
            {
              label: "Tarefas atrasadas",
              desc: "Avisar quando uma tarefa estiver atrasada",
              enabled: true,
            },
            {
              label: "Tarefas importantes",
              desc: "Notificar sobre tarefas de alta prioridade",
              enabled: true,
            },
            {
              label: "Lembretes diários",
              desc: "Resumo diário das tarefas do dia",
              enabled: false,
            },
            {
              label: "Tarefas agendadas",
              desc: "Lembrete antes de tarefas agendadas",
              enabled: true,
            },
          ].map((item) => (
      ...
      <div className="card p-5">
        <button
          ...
          className="w-full flex items-center gap-3 text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-xl p-3 transition-colors"
        >
      ...
      <p className="text-center text-xs text-slate-400 dark:text-slate-600">
        TaskFlow v1.0 • Feito com dedicação
      </p>
...
function Toggle({ defaultOn }: { defaultOn: boolean }) {
  ...
  return (
    ...
    >
      <span
        className={`absolute top-0.5 left-0.5 w-5 h-5 rounded-full bg-white shadow-sm transition-transform ${on ? "translate-x-6" : ""}`}
      />
    </button>
  );
}
