    ...
    <div className="p-4 lg:p-6 max-w-6xl mx-auto pb-24 lg:pb-6">
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-base font-semibold text-slate-900 dark:text-slate-100 ml-2 capitalize">
            {headerLabel}
          </h3>
        </div>
          ...
          {(["day", "week", "month"] as View[]).map((v) => (
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all capitalize ${
                view === v
                  ? "bg-white dark:bg-[#0b0b0b] text-green-600 dark:text-green-400 shadow-sm"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="lg:col-span-2 card p-4">
          ...
          {view === "day" && (
            <DayView selectedDate={selectedDate} tasks={selectedTasks} onEditTask={onEditTask} />
          )}
        </div>
    ...
    <div>
      <div className="grid grid-cols-7 gap-1 mb-2">
        {weekdays.map((d) => (
          <div
            key={d}
            className="text-center text-xs font-medium text-slate-400 dark:text-slate-500 capitalize py-1"
          >
            {d}
          </div>
        ))}
              ...
              {dayTasks.length > 0 && (
                <div className="flex gap-0.5 mt-0.5 flex-wrap justify-center">
                  {dayTasks.slice(0, 3).map((t) => (
                    <span
                      key={t.id}
                      className={`w-1 h-1 rounded-full ${isSelected ? "bg-white" : t.priority === "urgente" ? "bg-red-500" : t.priority === "alta" ? "bg-amber-500" : "bg-green-500"}`}
                    />
                  ))}
                </div>
              )}
      ...
      {days.map((day, i) => {
        ...
        return (
          ...
          >
            ...
            <span
              className={`text-lg font-bold mb-1 ${isSelected ? "text-white" : "text-slate-900 dark:text-slate-100"}`}
            >
              {day.getDate()}
            </span>
  ...
  if (tasks.length === 0) {
    return (
      <EmptyState
        ...
        message="Não há tarefas agendadas para este dia."
      />
    );
  }
  return (
