...
export function Dashboard({ onNavigate, onEditTask, onNewTask, onOpenAssistant }: DashboardProps) {
  ...
  const stats = useMemo(() => {
    ...
    const highPriority = tasks.filter(
      (t) => (t.priority === "alta" || t.priority === "urgente") && t.status === "pendente",
    );
  ...
  const insights = useMemo(() => {
    ...
    if (stats.overdue > 0) {
      result.push({
        icon: AlertTriangle,
        text: `Você tem ${stats.overdue} tarefa(s) atrasada(s). Priorize concluí-las hoje.`,
    ...
    if (stats.todayTasks.length > 0) {
      result.push({
        icon: Sun,
        text: `${stats.todayTasks.length} tarefa(s) para hoje. Mantenha o foco!`,
    ...
    if (stats.productivity >= 70) {
      result.push({
        icon: TrendingUp,
        text: `Excelente! Sua produtividade está em ${stats.productivity}%.`,
        color: "text-emerald-600 dark:text-emerald-400",
      });
    } else if (stats.total > 0) {
      result.push({
        icon: TrendingUp,
        text: `Sua produtividade está em ${stats.productivity}%. Continue!`,
    ...
    if (stats.highPriority.length > 2) {
      result.push({
        icon: Sparkles,
        text: `Você tem ${stats.highPriority.length} tarefas de alta prioridade. Considere usar o assistente IA para organizar.`,
        color: "text-emerald-600 dark:text-emerald-400",
      });
    }
    return result;
            ...
            <div className="relative w-32 h-32 mb-4">
              <svg className="w-full h-full -rotate-90" viewBox="0 0 120 120">
                <circle
                  ...
                  fill="none"
                  strokeWidth="10"
                  className="stroke-slate-200 dark:stroke-[#303030]"
                />
                <circle
                  cx="60"
                  cy="60"
        ...
        <div className="space-y-3">
          ...
          {insights.length === 0 ? (
            <div className="card p-6 text-center">
              ...
              <p className="text-sm text-slate-500 dark:text-slate-400">
                Adicione tarefas para receber insights personalizados.
              </p>
            </div>
          ) : (
            <div className="space-y-2">
          ...
          {recentActivity.length === 0 ? (
            ...
            <div className="space-y-2">
              {recentActivity.map((task) => (
                ...
                >
                  <div
                    className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${task.status === "concluida" ? "bg-emerald-100 dark:bg-emerald-900/30" : "bg-slate-100 dark:bg-[#1c1c1c]"}`}
                  >
                  ...
                  <div className="flex-1 min-w-0">
                    <p
                      className={`text-sm font-medium truncate ${task.status === "concluida" ? "text-slate-400 dark:text-slate-500 line-through" : "text-slate-900 dark:text-slate-100"}`}
                    >
                      {task.title}
                    </p>
                    <p className="text-xs text-slate-400 dark:text-slate-500">
                      {task.status === "concluida" ? "Concluída" : "Criada"} •{" "}
                      {relativeDate(task.due_date)}
                    </p>
                  </div>
                </div>
