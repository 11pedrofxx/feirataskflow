...
export function Modal({ open, onClose, title, children, size = "md" }: ModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 animate-fade-in">
      ...
      />
      <div
        className={`relative w-full ${SIZES[size]} max-h-[90vh] overflow-y-auto card animate-scale-in`}
      >
        {title && (
          <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 dark:border-[#2a2a2a] sticky top-0 bg-white dark:bg-[#0b0b0b] rounded-t-2xl z-10">
            ...
            <button
              onClick={onClose}
              className="p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-[#242424] transition-colors"
            >
