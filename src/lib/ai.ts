...
function parseDate(text: string): string | undefined {
  ...
  for (const [word, dayNum] of Object.entries(DAY_MAP)) {
    if (
      lower.includes(`próxima ${word}`) ||
      lower.includes(`proxima ${word}`) ||
      lower.includes(word)
    ) {
...
function parsePriority(text: string): Priority | undefined {
  const lower = text.toLowerCase();
  if (lower.includes("urgente") || lower.includes("emergência") || lower.includes("emergencia"))
    return "urgente";
...
function detectCategory(text: string): string | undefined {
  ...
  if (
    ...
    lower.includes("projeto da empresa")
  )
    return "Trabalho";
  if (
    lower.includes("estud") ||
    lower.includes("prova") ||
    lower.includes("trabalho da escola") ||
    lower.includes("projeto da escola") ||
    lower.includes("aula") ||
...
function parseTimeQuery(text: string): { hour: number; minute: number } | null {
  ...
  const periodMap: Record<string, { hour: number; minute: number }> = {
    manhã: { hour: 9, minute: 0 },
    manha: { hour: 9, minute: 0 },
    tarde: { hour: 14, minute: 0 },
    noite: { hour: 19, minute: 0 },
  };
  for (const [word, time] of Object.entries(periodMap)) {
    if (
      lower.includes(`pela ${word}`) ||
      lower.includes(`de ${word}`) ||
      lower.includes(`no ${word}`) ||
      lower.includes(`na ${word}`)
    ) {
      return time;
    }
  }
...
export function askAssistant(question: string, tasks: Task[]): AIResponse {
  ...
  if (
    ...
    const periodOnly = tasks.filter(
      (t) => !t.due_time && t.status === "pendente" && t.planned_period,
    );
  ...
  if (
    (lower.includes("o que") && (lower.includes("fazer hoje") || lower.includes("fazer agora"))) ||
    lower.includes("organize meu dia") ||
    lower.includes("organize a minha dia") ||
    lower.includes("organiza meu dia")
  ) {
    if (today.length === 0 && overdue.length === 0) {
      return {
        text: "Você não tem tarefas para hoje. Aproveite para planejar seus próximos passos ou adicionar novas tarefas.",
      };
    }
  ...
  if (
    lower.includes("prioriz") ||
    lower.includes("prioridade") ||
    lower.includes("mais importante") ||
    lower.includes("o que eu devo fazer primeiro")
  ) {
    ...
    const lines = top.map(
      (t, i) => `${i + 1}. ${t.title} — ${t.priority}${isOverdue(t) ? " (atrasada)" : ""}`,
    );
    return {
      text: `Recomendo priorizar estas tarefas:\n\n${lines.join("\n")}\n\nFoque primeiro nas urgentes e atrasadas.`,
    };
  }
  ...
  if (lower.includes("atrasa")) {
    ...
    return {
      text: `Você tem ${overdue.length} tarefa(s) atrasada(s):\n\n${lines.join("\n")}\n\nRecomendo priorizar estas tarefas o quanto antes.`,
    };
  }
  ...
  if (
    ...
    const conflicts = [...byDate.entries()]
      .filter(([, c]) => c > 3)
      .sort((a, b) => a[0].localeCompare(b[0]));
    if (conflicts.length === 0)
      return { text: "Não detectei conflitos de prazos. Sua agenda está equilibrada." };
    const lines = conflicts.map(([date, count]) => `• ${date}: ${count} tarefas`);
    return {
      text: `Detectei possíveis conflitos de prazo:\n\n${lines.join("\n")}\n\nConsidere redistribuir algumas tarefas ou adiantar as mais importantes.`,
    };
  }
  ...
  if (lower.includes("produtiv") || lower.includes("dica") || lower.includes("recomenda")) {
    ...
    const tips: string[] = [];
    if (overdue.length > 0)
      tips.push(`Você tem ${overdue.length} tarefa(s) atrasada(s). Priorize concluí-las hoje.`);
    if (today.length > 5)
      tips.push(
        `Você tem ${today.length} tarefas para hoje. Considere adiar as de menor prioridade.`,
      );
    ...
    return {
      text: `Aqui estão algumas recomendações de produtividade:\n\n${tips.map((t) => `• ${t}`).join("\n")}`,
    };
  }
